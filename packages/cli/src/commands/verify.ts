...
export async function runUiVerification(options: { cwd?: string; contractId?: string; runner?: PlaywrightRunner } = {}): Promise<VerifyRunResult> {
  ...
  try {
    const filters = await Promise.all(runnable.map(async (item) => playwrightPathFilter(await realpath(item.specPath))));
    const args = ["test", "--reporter=json", "--", ...filters];
...
function ownsGeneratedSpec(content: string, contractId: string): boolean {
  return normalizeLineEndings(content).startsWith(`// @generated by LogicGraph\n// logicgraph-ui-contract: ${contractId}`);
}
...
function playwrightPathFilter(specPath: string): string {
  return `^${specPath.split(sep).map(escapeRegExp).join("[/\\\\]")}$`;
}
