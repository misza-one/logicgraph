...
export type VerifyScaffoldStatus = "generated" | "unchanged" | "needs-route" | "skipped";
...
export async function verifyScaffoldCommand(contractId?: string): Promise<void> {
  try {
    ...
    if (result.items.some((item) => item.status === "needs-route")) {
      process.exitCode = 1;
    }
  } catch (error) {
...
export function formatScaffoldResult(result: VerifyScaffoldResult): string {
  ...
  for (const item of result.items) {
    const mark = item.status === "needs-route" ? "✗" : item.partialReasons.length > 0 ? "⚠" : "✓";
    lines.push(`${mark} ${item.contractId}  ${item.status === "needs-route" ? item.reason : item.specRelativePath}`);
